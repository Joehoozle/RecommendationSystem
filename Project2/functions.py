...
def itemSimCalc(a,b,avg,user):
    ...
    #add the data from one new user
    tracker = tracker + ((a[user] - avg[user]) * (b[user] - avg[user]))
    sizeA = sizeA + ((a[user] - avg[user]) ** 2)
    sizeB = sizeB + ((b[user] - avg[user]) ** 2)
